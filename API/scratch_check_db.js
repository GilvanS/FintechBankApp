...
async function check() {
    try {
        ...
        const overdue = await db.executeQuery("SELECT cpf, valor_total, status, data_pagamento FROM fintechbank.default.invoices WHERE status = 'FECHADA' AND data_pagamento IS NULL LIMIT 10");
        console.table(overdue);
        
    } catch(err) {
        console.error("ERROR running databricks queries:", err);
    }
}
check();
