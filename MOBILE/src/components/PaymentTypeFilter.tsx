...
interface PaymentTypeFilterProps {
    ...
    showDots?: boolean;
    className?: string;
    /** Tema visual dos chips. 'volt' = verde limao (padrao), 'neon' = ciano/azul, 'mono' = monocromatico */
...
/**
 * Chips de filtro por tipo de pagamento.
 * Portado de WEB/components/PaymentTypeFilter.tsx + feedback tatil nativo ao trocar filtro.
 */
const PaymentTypeFilter: React.FC<PaymentTypeFilterProps> = ({
    ...
    const handleSelect = (f: PaymentFilterValue, isActive: boolean) => {
        if (Capacitor.isNativePlatform()) {
            Haptics.impact({ style: ImpactStyle.Light }).catch(() => { /* dispositivo sem haptics */ });
        }
        onFilterChange(isActive ? 'ALL' : f);
    };
        ...
        <div className={`flex gap-1.5 overflow-x-auto hide-scrollbar ${className}`}>
            {filters.map(f => {
                ...
                return (
                    <button
                        key={f}
                        onClick={() => handleSelect(f, isActive)}
