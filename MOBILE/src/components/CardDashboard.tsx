...
import { CreditCard, Key, ShieldAlert, Sliders, ToggleLeft, ToggleRight, CheckCircle2, AlertCircle, ArrowLeft } from 'lucide-react';
...
const CardDashboard: React.FC<CardDashboardProps> = ({ onBack, onNavigate }) => {
    ...
    const [tempLimit, setTempLimit] = useState(creditCard.totalLimit || 1000);
    ...
    const toggleNfc = () => {
        updateUser({
            ...
            creditCard: {
                ...creditCard,
                isNfcEnabled: !creditCard.isNfcEnabled
            }
        });
    };
    ...
    return (
        <div className={`min-h-screen relative pb-40 ${theme === 'midnight' ? 'bg-[#0f0f0f]' : 'bg-volt-yellow'}`}>
            ...
            <div className="space-y-6 pt-2 px-4 max-w-md mx-auto">
                            ...
                            <div className="space-y-4">
                                <div className="flex items-center gap-3">
                                    ...
                                    <div className="text-white/85 font-mono tracking-widest text-base drop-shadow-[0_1px_2px_rgba(0,0,0,0.5)]">
                                        •••• •••• •••• {String(creditCard.number || '0000').slice(-4).padStart(4, '0')}
                                    </div>
                                </div>
                                ...
                                <div className="flex justify-between items-end">
                                    <div className="space-y-0.5">
                                        ...
                                        <p className="text-white font-bold tracking-widest uppercase text-sm drop-shadow-[0_1px_2px_rgba(0,0,0,0.5)]">
                                            {String(user.fullName || 'Usuário').split(' ')[0]}
                                        </p>
                                    </div>
                ...
                <div className="bg-volt-surface border border-white/5 rounded-2xl p-4 flex justify-between items-center">
                    <div>
                        <p className="text-[10px] uppercase tracking-widest text-on-surface-variant font-bold">Fatura Atual</p>
                    ...
                    {[
                        {
                            label: 'Ver fatura',
                            icon: CreditCard,
                            action: () => onNavigate('currentInvoice'),
                        },
                        {
                            label: 'Meus limites',
                            icon: Sliders,
                            action: () => {
                                setTempLimit(creditCard.totalLimit || 1000);
