...
export default function AiRecurringBillModal({
  ...
  const fetchAnalysis = async () => {
    ...
    setResult(null);
    try {
      // Simulate network / AI thinking time (match 4 loading steps of 2200ms)
      await new Promise(resolve => setTimeout(resolve, 8800));

      // Deterministic Logic: Detect recurring charges
      const txMap = new Map<string, Transaction[]>();
      transactions.forEach(tx => {
        if (tx.amount < 0 && tx.type === 'PAYMENT') {
           const desc = tx.description.trim().toLowerCase();
           if (!txMap.has(desc)) txMap.set(desc, []);
           txMap.get(desc)!.push(tx);
        }
      });

      const missedSubscriptions: MissedSubscription[] = [];
      
      txMap.forEach((txs, desc) => {
        // If the same exact charge happens more than once (i.e. different months, we just check count >= 2 here for simplicity)
        if (txs.length >= 2) {
          const isRegistered = recurringBills.some((b: any) => b.title.trim().toLowerCase() === desc);
          if (!isRegistered) {
            missedSubscriptions.push({
              title: txs[0].description,
              estimatedAmount: txs[0].amount, // usually negative
              frequency: 'Mensal (Estimado)',
              reason: 'Encontramos débitos repetidos nos últimos meses que não estão no seu controle de Assinaturas.'
            });
          }
        }
      });

      const potentialCancellations: PotentialCancellation[] = [];
      let totalSavingsPotential = 0;

      if (missedSubscriptions.length > 0) {
         // Create a fake optimization opportunity from the highest unregistered bill
         const highest = missedSubscriptions.reduce((prev, current) => 
            (Math.abs(current.estimatedAmount) > Math.abs(prev.estimatedAmount)) ? current : prev
         );
         const savingPotential = parseFloat((Math.abs(highest.estimatedAmount) * 0.35).toFixed(2));
         
         potentialCancellations.push({
           title: highest.title,
           amount: highest.estimatedAmount,
           reason: 'Existem planos familiares ou anuais que podem reduzir este custo em até 35%. Considere renegociar.',
           savingPotential
         });
         
         totalSavingsPotential = savingPotential;
      }

      setResult({
        analysis: "Analisei suas transações recentes. Notei que você possui algumas cobranças frequentes que estão fora do seu monitoramento de assinaturas e oportunidades para economizar.",
        missedSubscriptions,
