...
const LAST_SHOWN_KEY = 'overdue_alert_last_shown';

/**
 * REGRA DE NEGOCIO: o alerta trata de uma fatura fechada COM saldo a pagar.
 * Quem quitou saiu do atraso e nao deve mais ve-lo. O saldo devedor e a unica
 * condicao necessaria: sem divida nao ha o que pagar nem o que alertar.
 *
 * Portado de WEB/components/OverdueAlertModal.tsx. A checagem de janela de 1h
 * usa @capacitor/preferences (async) no lugar de localStorage.
 */
export function hasOverdueDebt(user?: User): boolean {
...
export async function shouldShowOverdueAlert(user?: User): Promise<boolean> {
  if (!hasOverdueDebt(user)) return false;

  try {
    const { value } = await Preferences.get({ key: LAST_SHOWN_KEY });
    if (!value) return true;

    const lastShown = parseInt(value, 10);
...
export async function recordOverdueAlertShown(): Promise<void> {
  try {
    await Preferences.set({ key: LAST_SHOWN_KEY, value: Date.now().toString() });
  } catch {
    // Silencio
  }
}
...
const OverdueAlertModal: React.FC<OverdueAlertModalProps> = ({
  ...
  useEffect(() => {
    let cancelled = false;
    (async () => {
      const show = await shouldShowOverdueAlert(user);
      if (cancelled || !show) return;
      setOpen(true);
      await recordOverdueAlertShown();
      if (Capacitor.isNativePlatform()) {
        Haptics.notification({ type: NotificationType.Warning }).catch(() => { /* sem haptics */ });
      }
    })();
        ...
        <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 bg-black/80 backdrop-blur-md">
          ...
          >
            ...
            <p className="text-xs text-zinc-300 leading-relaxed mb-4">
              Sua fatura anterior nao foi paga. O valor foi transferido para a proxima fatura
              (aberta), que agora inclui tambem os encargos por atraso.
            </p>

            {/* Sessao 1: Fatura Fechada em Atraso */}
            <div className="bg-zinc-900/90 rounded-2xl border border-rose-500/20 p-4 mb-3">
              ...
              <p className="text-[10px] text-zinc-500 mt-2 leading-relaxed">
                Pague este valor para regularizar sua conta. Apos o pagamento, os encargos serao recalculados na proxima fatura.
              </p>
            </div>

            {/* Aviso: encargos foram para a proxima fatura */}
            <div className="bg-zinc-900/90 rounded-2xl border border-amber-500/20 p-3.5 mb-4">
              <p className="text-[11px] text-zinc-400 leading-relaxed">
                ...
                foram transferidos para a <span className="text-white font-semibold">proxima fatura (aberta)</span>.
