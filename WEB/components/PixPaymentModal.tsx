...
export default function PixPaymentModal({
  ...
  const [paymentCodes, setPaymentCodes] = useState<PaymentCodesResponse['data'] | null>(null);
  ...
  // Detecta bandeira do cartão a partir dos dados reais
  ...
  // Gera os códigos de pagamento ao abrir o modal
  useEffect(() => {
    if (!isOpen) return;
    setFromCache(false);
    setStep('generating');
    ...
    (async () => {
      ...
      const cached = getPaymentCodesFromCache(user.cpf!, invId);
      ...
      try {
        ...
        if (res.success && res.data) {
          savePaymentCodesToCache(user.cpf!, invId, res.data);
          setPaymentCodes(res.data);
          setStep('show_pix');
        } else {
          setErrorMessage('Falha ao gerar código PIX. Tente novamente.');
          setStep('error');
        }
      } catch (err: any) {
  ...
  const handleShare = async () => {
    ...
    if (navigator.share) {
      try {
        await navigator.share({ title: 'Código PIX', text: paymentCodes.pix.payload });
      } catch {}
    } else {
  ...
  return (
    <div className="fixed inset-0 z-[100] flex flex-col bg-white font-sans max-w-md mx-auto">
      {/* Header — estilo do projeto (lime green #A2FF00) */}
      ...
      <div className="flex-1 overflow-y-auto px-5 pt-6 pb-4 space-y-6">
        ...
        {step === 'generating' && (
          <div className="flex flex-col items-center justify-center py-24 gap-4">
            ...
            <p className="text-sm font-semibold text-slate-700 text-center">Gerando código PIX...</p>
        ...
        {/* Show PIX - Santander style */}
        {step === 'show_pix' && (
          <>
            {/* Subtitle */}
            <p className="text-xs text-slate-600 text-center leading-tight px-2">
              Libere seu limite pagando o valor total ou acima do mínimo para evitar bloqueios
            </p>
            ...
            <div className="space-y-3 bg-white rounded-xl p-4 border-2 border-black shadow-[3px_3px_0px_0px_rgba(0,0,0,1)]">
              <div className="flex justify-between items-center">
                <div>
                  <p className="text-[10px] text-black/50 uppercase tracking-wider font-semibold">Cartão de crédito</p>
              ...
              <div className="border-t border-black/10 pt-3 space-y-2">
                ...
                <div className="flex justify-between items-center">
                  <p className="text-xs text-black/50">Valor mínimo a ser pago</p>
            ...
            <div className="flex items-start gap-3 bg-blue-50 border border-blue-200 rounded-lg p-3">
              ...
              <p className="text-xs text-blue-800 leading-tight">
                Lembre-se de digitar o valor do pagamento quando for usar o código
              </p>
            </div>
            ...
            <div className="space-y-3">
              ...
              >
                <Share2 size={18} /> Compartilhar código
              </button>
            </div>
            ...
            {fromCache && (
              <p className="text-[10px] text-amber-600 text-center font-semibold">
                Código recuperado do cache local
              </p>
            )}
          </>
        )}
