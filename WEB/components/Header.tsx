...
export default function Header({
  ...
  const handleLogoClick = () => {
    ...
    if (now - lastClickTime < 500) {
      if (clickCount + 1 >= 3) {
        onThemeToggle(theme === 'yellow' ? 'midnight' : 'yellow');
        setClickCount(0);
      } else {
  ...
  const getTitle = () => {
    switch (currentView) {
      ...
      case 'currentInvoice':
      case 'closedInvoice': return 'Fatura';
      default: return 'VOLT';
    }
  };
  ...
  return (
    <>
      <header className={`fixed top-2 left-2 right-2 z-40 rounded-2xl h-16 px-4 flex justify-between items-center shadow-2xl backdrop-blur-md border border-white/5 ${theme === 'midnight' ? 'bg-volt-surface/80 text-white' : 'bg-white/80 text-black'}`}>
        {/* Left Side */}
        <div className="flex items-center gap-3">
          ...
          {showBackButton ? (
            <button
              onClick={handleBack}
              className={`p-1.5 rounded-full transition-colors active:scale-90 border border-transparent hover:border-white/10 ${theme === 'midnight' ? 'bg-white/5 hover:bg-white/10' : 'bg-black/5 hover:bg-black/10'}`}
            >
              <ArrowLeft size={16} />
            </button>
          ) : (
            <div className="w-10 h-10 flex items-center justify-center font-black text-lg text-black bg-volt-green rounded-full overflow-hidden active:scale-95 transition-transform cursor-pointer shadow-lg" onClick={() => setProfileOpen(true)}>
              {avatarFallback}
            </div>
          )}

          <h1
            onClick={getTitle() === 'VOLT' ? handleLogoClick : undefined}
            className={`font-black tracking-tighter uppercase transition-all duration-200 select-none ${getTitle() === 'VOLT' ? 'cursor-pointer' : ''} ${
              getTitle() === 'VOLT'
                ? 'text-2xl italic text-volt-green tracking-tight font-black'
                : `text-base font-black ${theme === 'midnight' ? 'text-white' : 'text-black'}`
            }`}
          >
            {getTitle()}
          </h1>
        ...
        <div className="flex items-center gap-2">
          ...
          <button
            ...
            className="w-8 h-8 rounded-full flex items-center justify-center text-black bg-volt-green hover:brightness-95 shadow-lg transition-all active:scale-90 cursor-pointer animate-pulse"
          ...
          {currentView !== 'profile' && !showBackButton && (
            <div className="w-8 h-8 flex items-center justify-center font-black text-black rounded-full bg-volt-surface-high overflow-hidden border-2 border-black ml-1 active:scale-95 transition-transform cursor-pointer" onClick={() => onNavigate('profile')}>
              {avatarFallback}
            </div>
          )}
        </div>
          ...
          <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
            ...
            >
              ...
              <div className="flex flex-col items-center space-y-2">
                <div className="w-20 h-20 flex items-center justify-center font-black text-4xl text-white rounded-full border-2 border-volt-green/50 p-1">
                  {avatarFallback}
                </div>
              ...
              <div className="grid grid-cols-2 gap-2 text-left">
                <div className="p-3 bg-white/5 rounded-xl space-y-1">
                  ...
                  <p className="text-xs font-bold text-white">{user?.accountStatus === 'inadimplente' ? 'Bloqueada' : 'Ativa'}</p>
                </div>
                <div className="p-3 bg-white/5 rounded-xl space-y-1">
                  ...
                  <p className="text-xs font-bold text-white">{user?.creditCard?.pointsBalance || 0} PTS</p>
                </div>
              </div>
              ...
              <div className={`grid grid-cols-3 gap-3 p-3.5 rounded-2xl border-2 border-black/10 dark:border-white/10 ${
                ...
                <div className="text-center">
                  ...
                  <span className="text-sm font-black text-volt-green mt-0.5 block">
                    R$ {(transactions || []).reduce((acc, t) => acc + (t.type === 'DEPOSIT' || t.type === 'PIX_RECEIVED' || t.type === 'CASHBACK_CREDIT' ? t.amount : -Math.abs(t.amount)), 0).toLocaleString('pt-BR', { minimumFractionDigits: 2 })}
                  </span>
                </div>
