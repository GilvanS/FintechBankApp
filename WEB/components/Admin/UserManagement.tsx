...
import { adminGetUserByCpf, blockUser, unblockUser, adminDeposit, adminUpdateUserPassword, adminUpdateCreditLimit, adminAdjustCreditLimit, adminUpdatePixLimit } from '../../services/api';
        ...
        try {
            switch (modalState.action) {
                ...
                case 'creditLimitAdjust':
                    ...
                    result = await adminAdjustCreditLimit(modalState.data.cpf, adjustAmt);
                    break;
                case 'pixLimit':
                        ...
                        <div className="bg-black/5 dark:bg-white/5 p-6 rounded-2xl border border-black/10 dark:border-white/10 space-y-6">
                            ...
                            <div className="grid grid-cols-2 gap-4 pt-4 border-t border-black/10 dark:border-white/10">
                                <div>
                                    ...
                                    <p className="text-xl font-bold">R$ {Number(searchedUser.balance || 0).toFixed(2).replace('.', ',')}</p>
                                </div>
                                <div>
                                    <p className="text-[10px] opacity-70 font-bold uppercase mb-1">Limite Crédito (Total)</p>
                                    <p className="text-xl font-bold">R$ {Number(searchedUser.creditCard?.totalLimit || searchedUser.creditCard?.limit || 5000).toFixed(2).replace('.', ',')}</p>
                                </div>
                                <div>
                                ...
                                <div>
                                    ...
                                    <p className="text-lg font-bold">R$ {Number(searchedUser.pixDailyLimit || (searchedUser as any).dailyPixLimit || 2000).toFixed(2).replace('.', ',')}</p>
                                </div>
                                <div>
                                    <p className="text-[10px] opacity-70 font-bold uppercase mb-1">Dia Vencimento</p>
                                    <p className="text-lg font-bold">Dia {searchedUser.creditCard?.dueDay || searchedUser.billingDay || 10}</p>
                                </div>
                            </div>
                        </div>
