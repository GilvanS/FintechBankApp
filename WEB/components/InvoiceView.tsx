              ...
              <div className={`space-y-3 py-3 px-4 rounded-2xl text-xs border ${
                ...
                <div className="flex justify-between items-center">
                  ...
                  <span className="font-black font-mono">
                    {(() => {
                      const num = selectedTx.cardNumber || selectedTx.cardLast4 || user.creditCard?.number || '';
                      if (!num || num.replace(/\D/g, '') === '0000000000000000' || num.replace(/\D/g, '') === '0000') {
                        return '5981 1475 1088 1435';
                      }
                      return num;
                    })()}
                  </span>
                ...
                {(() => {
                  let current = selectedTx.currentInstallment;
                  let total = selectedTx.totalInstallments;
                  if ((!current || !total) && selectedTx.installments && typeof selectedTx.installments === 'string') {
                    const match = selectedTx.installments.match(/(\d+)\s*[\/de]+\s*(\d+)/i);
                    if (match) {
                      current = Number(match[1]);
                      total = Number(match[2]);
                    }
