...
const SECTIONS: readonly AllureSection<SubSectionKey>[] = [
  ...
  { key: 'insights', label: 'Análise de Orçamento', icon: Activity },
];
...
export function LimitsAllureView({ user, theme, onBack, onNavigate }: Props) {
  ...
  return (
    <AllureShell
      ...
      subtitle="Aumento de limite, gamificação de gastos e controle financeiro Volt"
      theme={theme}
      onBack={onBack}
      sections={SECTIONS}
    ...
    >
      {/* REGRA DE OURO: Reaproveita o LimitView.tsx real em formato Allure de 2 colunas com todos os 5 modais nativos */}
      <div className="w-full">
        {user ? (
          ...
          <div className="p-8 text-center text-xs opacity-60">Carregando dados...</div>
        )}
      </div>
    </AllureShell>
  );
